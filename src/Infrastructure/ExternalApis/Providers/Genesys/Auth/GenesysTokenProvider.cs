...
public sealed class GenesysTokenProvider : IGenesysTokenProvider
{
    ...
    {
        GenesysTokenCacheEntry? entry = await _tokenStore.TryGetValidAsync(LobKey, ct)
                                                         .ConfigureAwait(false);

        if (entry is not null) return entry.AccessToken;

        await using IAsyncDisposable gate = await _keyedLock.AcquireAsync(LockKey, ct)
                                                            .ConfigureAwait(false);

        entry = await _tokenStore.TryGetValidAsync(LobKey, ct)
                                 .ConfigureAwait(false);

        if (entry is not null) return entry.AccessToken;

        GenesysTokenCacheEntry fresh = await FetchFreshTokenAsync(ct)
           .ConfigureAwait(false);
        await _tokenStore.UpsertAsync(LobKey, fresh, ct)
                         .ConfigureAwait(false);
    ...
    private async Task<GenesysTokenCacheEntry> FetchFreshTokenAsync(CancellationToken ct)
    {
        ...
        GenesysTokenResponse response =
            await _tokenApiClient.RequestClientCredentialsTokenAsync(clientId, clientSecret, ct)
                                 .ConfigureAwait(false);
