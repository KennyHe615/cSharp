...
public sealed class GenesysApiClient : IGenesysApiClient
{
        ...
        CancellationToken ct)
    {
        ...
        try
        {
            return await operation(BuildHeaders(headers, token), ct).ConfigureAwait(false);
        }
        catch (ExternalServiceHttpException ex) when (ex.StatusCode == HttpStatusCode.Unauthorized)
        {
            await _tokenProvider.RefreshTokenAsync(ct).ConfigureAwait(false);
            string refreshedToken = await _tokenProvider.GetValidTokenAsync(ct).ConfigureAwait(false);

            return await operation(BuildHeaders(headers, refreshedToken), ct).ConfigureAwait(false);
        }
    }
        ...
        CancellationToken ct)
    {
        ...
        try
        {
            await operation(BuildHeaders(headers, token), ct).ConfigureAwait(false);
        }
        catch (ExternalServiceHttpException ex) when (ex.StatusCode == HttpStatusCode.Unauthorized)
        {
            await _tokenProvider.RefreshTokenAsync(ct).ConfigureAwait(false);
            ...
            await operation(BuildHeaders(headers, refreshedToken), ct).ConfigureAwait(false);
        }
    }
