...
namespace Infrastructure.ExternalApis.Shared.Policies;
...
public sealed class HttpResiliencePolicyFactory : IHttpResiliencePolicyFactory
{
    ...
    private IAsyncPolicy BuildRetryPolicy(RetryStrategy strategy, string kind)
    {
        ...
        return Policy.Handle<Exception>(ex => ShouldRetry(ex, strategy.StatusCodes))
                     .WaitAndRetryAsync(strategy.MaxAttempts,
                                        (attempt, ex, _) => CalculateDelay(attempt, ex, strategy),
                                        async (ex, delay, attempt, context) =>
                                        {
                                            string lob =
                                                context.TryGetValue(HttpPolicyContextKeys.Lob, out object? lobObj)
                                            ...
                                            bool hasRefresh =
                                                context.TryGetValue(HttpPolicyContextKeys.RefreshFunc,
                                                                    out object? refreshObj)
    ...
    {
        return Policy.Handle<Exception>(ex => ShouldBreak(GetStatusCode(ex)))
