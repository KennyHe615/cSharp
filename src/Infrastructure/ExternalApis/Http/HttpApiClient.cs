...
public class HttpApiClient : IHttpApiClient
{
        ...
        return ExecuteWithPolicyAsync(_safePolicy,
                                      ...
                                      async (req, token) =>
                                      {
                                          using IFlurlResponse res =
                                              await req.GetAsync(cancellationToken: token).ConfigureAwait(false);
                                                           ...
                                                           CancellationToken ct = default)
    {
        return ExecuteWithPolicyAsync(_unsafePolicy,
                                      ...
                                      async (req, token) =>
                                      {
                                          ...
                                          TResponse data = await res.GetJsonAsync<TResponse>().ConfigureAwait(false);
                                                           ...
                                                           CancellationToken ct = default)
    {
        return ExecuteWithPolicyAsync(_unsafePolicy,
                                      ...
                                      async (req, token) =>
                                      {
                                          using IFlurlResponse res =
                                              await req
                                                   .PostUrlEncodedAsync(payload, cancellationToken: token)
                                                   .ConfigureAwait(false);
                                          TResponse data = await res.GetJsonAsync<TResponse>().ConfigureAwait(false);
                                                          ...
                                                          CancellationToken ct = default)
    {
        return ExecuteWithPolicyAsync(_unsafePolicy,
                                      ...
                                      async (req, token) =>
                                      {
                                          using IFlurlResponse res =
                                              await req
                                                   .PutJsonAsync(payload, cancellationToken: token)
                                                   .ConfigureAwait(false);
                                          TResponse data = await res.GetJsonAsync<TResponse>().ConfigureAwait(false);
                                                            ...
                                                            CancellationToken ct = default)
    {
        return ExecuteWithPolicyAsync(_unsafePolicy,
                                      ...
                                      async (req, token) =>
                                      {
                                          using IFlurlResponse res =
                                              await req
                                                   .PatchJsonAsync(payload, cancellationToken: token)
                                                   .ConfigureAwait(false);
                                          TResponse data = await res.GetJsonAsync<TResponse>().ConfigureAwait(false);
    ...
    public Task HeadAsync(string endpoint, Dictionary<string, string>? headers = null, CancellationToken ct = default)
    {
        return ExecuteWithPolicyAsync<object?>(_safePolicy,
                                               ...
                                               async (req, token) =>
                                               {
                                                   using IFlurlResponse res =
                                                       await req
                                                            .HeadAsync(cancellationToken: token)
                                                            .ConfigureAwait(false);
                                                     ...
                                                     CancellationToken cancellationToken = default)
    {
        ...
        return await policy
           .ExecuteAsync<T?>((_, ct) => ExecuteRequestCoreAsync(endpoint,
                                                                method,
                                                                fullUrl,
                                                                operation,
                                                                headers,
                                                      ...
                                                      CancellationToken ct)
    {
        try
        {
            ...
            (T? result, int statusCode) = await operation(request, ct).ConfigureAwait(false);
        ...
        {
            string? responseSummary = await TryGetSafeResponseSummaryAsync(ex).ConfigureAwait(false);
    ...
    private static async Task<string?> TryGetSafeResponseSummaryAsync(FlurlHttpException ex)
    {
        try
        {
            string body = await ex.GetResponseStringAsync().ConfigureAwait(false);
